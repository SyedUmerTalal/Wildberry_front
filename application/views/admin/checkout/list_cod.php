...
<div class="content-wrapper">
  <section class="content-header">
    <h1>
      <?php echo !empty($title)?$title:'COD Order Details';?>
    </h1>
  </section>
            ...
            <table id="datatable" class="table table-bordered table-striped">
              ...
              <tbody>            
                ...
                <tr>
                  ...
                  <td><?php echo !empty($record->cod_amount)?$record->cod_amount:'';?></td>
                  <td><?php echo !empty($record->checkout_created_at)?$record->checkout_created_at:'';?></td>
                  <td><?php echo !empty($record->rider)?$record->rider:'';?></td>
                  ...
                  <td>
                    <a href="<?php echo !empty($record->$record_id)?base_url('admin/'.$table_name.'/view_cod/').$record->$record_id:'';?>"><span class="view_icon"><i class="fa fa-eye" aria-hidden="true"></i></span></a>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="remarksmodalLabel">Assign Order to Rider</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="<?= base_url('admin/checkout/send_remarks');?>" method="post">
          <input type="hidden" id="hiddenID" name="checkout_id">
          <label>Action</label>
          <select class="form-control" required="" name="rider">
            <option value="pending" disabled selected="">Pending</option>
            <?php foreach($riders as $d):?>
            <option value="<?= $d->master_admin_name;?>"><?= $d->master_admin_name;?></option>
            <?php endforeach;?>
          </select> 
      </div>
